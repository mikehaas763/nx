...
export type TestTargetConfiguration = TargetConfigurationWithName & {
   ...
   *
   * Requires Microsoft.Testing.Platform, and requires Nx Cloud at run time: Nx
   * refuses to run atomized tasks without it, because splitting a suite across
   * one machine is slower than not splitting it at all.
   */
  ciTargetName?: string;
